...
import type { TakeScreenshotParams } from "../../../browser/dist/mod.js";
...
export async function takeScreenshot(
	...
	let { target_filepath } = reqParams;

	let response = await fetch(
		new URL(`/session/${sessionId}/screenshot`, webdriverUrl),
		{
			method: "GET",
			headers,
			signal,
...
async function getRequestParams(
	req: IncomingMessage,
): Promise<TakeScreenshotParams | undefined> {
	let json = await getJsonFromRequestBody(req);
	let { target_filepath } = json;
	if ("string" === typeof target_filepath) {
